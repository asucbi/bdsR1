...
```{r message=FALSE}
airbnb_badnames <- read_csv("data/airbnb-badnames.csv")
glimpse(airbnb_badnames)
```
...
```{r error=TRUE}
ggplot(airbnb_badnames, aes(x = Number of bathrooms, y = Price)) +
  geom_point()
```
...
.small[
```{r}
airbnb_col_names <- read_csv("data/airbnb-badnames.csv",
...
```{r warning=FALSE}
airbnb_clean_names <- read_csv("data/airbnb-badnames.csv") %>%
  janitor::clean_names()

names(airbnb_clean_names)
```
...
```{r message=TRUE, output.lines=7, echo=FALSE}
# .question[
...
```{r warning=FALSE}
fav_food <- read_excel("data/favorite-food.xlsx", na = c("N/A", "99999")) %>%
  janitor::clean_names() %>%
  mutate(
    age = if_else(age == "five", "5", age), 
    age = as.numeric(age),
