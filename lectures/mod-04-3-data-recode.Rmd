...
rel_inc <- rel_inc %>%
  rowwise() %>%
  mutate(n = sum(c_across(2:last_col()), na.rm = TRUE)) %>%
...
## Reverse `income` plotting order

.panelset[

.panel[.panel-name[Plot]
```{r rel-income-rev, out.width = "65%", echo = FALSE}
ggplot(rel_inc_long, aes(y = religion, x = frequency, fill = fct_rev(income))) +
  geom_col()
```
]

.panel[.panel-name[Code]
```{r ref.label = "rel-income-rev", fig.show = "hide"}
```
]
...
rel_inc_long %>%
  mutate( #<<
    religion = fct_reorder(religion, as.integer(income)*proportion, mean) #<<
    ) %>% #<<
  ggplot(aes(y = religion, x = frequency, fill = fct_rev(income))) +
  geom_col(position = "fill") 
...
## Change theme and swap palette direction
