...
## Fill with a categorical variable (overlaid)

.panelset[
.panel[.panel-name[Plot]
```{r ref.label = "hist-fill-over", echo = FALSE, warning = FALSE}
```
]
.panel[.panel-name[Code]
```{r hist-fill-over, fig.show = "hide", warning = FALSE}
ggplot(loans, aes(x = loan_amount, 
                  fill = homeownership)) + #<<
  geom_histogram(binwidth = 5000,
                 alpha = 0.5,
...
## Adding a categorical variable (y) 

.panelset[
.panel[.panel-name[Plot]
```{r ref.label = "box-cat-h", echo = FALSE, warning = FALSE}
```
]
.panel[.panel-name[Code]
```{r box-cat-h, fig.show = "hide", warning = FALSE}
ggplot(loans, aes(x = interest_rate,
                  y = grade)) + #<<
  geom_boxplot() +
  labs(
