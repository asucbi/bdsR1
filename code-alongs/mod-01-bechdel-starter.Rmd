...
INSTRUCTOR NOTE: review R Studio / Posit windows; cmd enter to run; up arrow to go back to previous; gear icon to "preview in viewer pane"
...
## STOPPED HERE ## (show down arrow to run above code)
...
bechdel90_13 %>%
  arrange(desc(roi)) %>% 
  select(title, roi, year, clean_test) # do we want all the columns? let's return the columns that are most important here
...
ggplot(data = bechdel90_13_zoom400, 
       mapping = aes(x = clean_test, y = roi, color = binary)) +
  geom_boxplot() +
  labs(title = "Return on investment vs. Bechdel test result",
       x = "Detailed Bechdel result",
...
```
**NOTE: BEFORE WORKING ON THE LAB, RUN THE FOLLOWING CODE**
