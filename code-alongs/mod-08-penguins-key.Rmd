...
penguins_sc %>% 
  ggplot(aes(x=body_mass_kg, y=bill_length_cm, color=species, shape=species)) +
  geom_point() +
  facet_wrap(~ sex)
...
```{r mod_flip, eval=FALSE}

mod_flip <- lm(bill_length_cm ~ flipper_length_cm, data=penguins_sc)
summary(mod_flip)
