...
```{r load-and-merge-data, eval = FALSE}

eiw <- read_csv("code-alongs/data/data1_processed.csv")
questions <- read_csv("code-alongs/data/study1_questions.csv")
...
```{r, fig.width=14, fig.height=9, eval = FALSE}
...
```{r plot-length-and-satisfaction, eval = FALSE}
...
```{r plot-log-length-and-satisfaction, eval = FALSE}
...
min_word_length <- function(x){
  min(nchar(unlist(strsplit(x, '\\W')))) 
}
...
```{r plot-avg-length, eval = FALSE}

eiw %>% mutate(text_length = stringr::str_length(text),
  word_length = avg_word_length(text)) %>%  # avg_word_length can be used as-is since it works on an entire character vector
  ggplot(aes(x=log(word_length), y=satisfaction)) +
    geom_point(alpha=.3) + 
    geom_smooth(method="lm") +
    geom_smooth()
...
```{r plot-max-length, eval = FALSE}

eiw %>% mutate(
  text_length = stringr::str_length(text),
  max_word_length = map_dbl(text, max_word_length)) %>% # need to use map_dbl because it operates on one string at a time and returns a numeric "double" vector
...
Then, let's run two regressions predicting satsifaction from: (1) average word length and (2) max word length.
...
cor.test(corr_df$satisfaction, log(corr_df$text_length)) # r=.5; p<.001
cor.test(corr_df$satisfaction, corr_df$word_length) # r=.06; p=.04
cor.test(corr_df$satisfaction, corr_df$max_word_length) # r=.36; p<.001 

```

## Are more mechanistic explanations more satisfying?

```{r mech-satisying, eval = FALSE}
...
cor.test(corr_df$satisfaction, corr_df$mechanism) # r=.61; p<.001
...
```{r control-for-length, eval = FALSE}
...
```{r analysis-control-length, eval = FALSE}

#First, let's run a regression predicting satisfaction from mechanism

mech_mod <- lm(satisfaction ~ mechanism, data = eiw)
mech_mod
str(mech_mod)
summary(mech_mod) #p<.001; for each 1-unit increase in mechanism, we predict a 0.53 increase in satisfaction
...
mech_length_mod <- lm(satisfaction ~ mechanism + log(text_length), data = eiw)
summary(mech_length_mod)
