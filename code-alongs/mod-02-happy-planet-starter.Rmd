...
View the dataset by typing `View(happy)` in the console or clicking on the dataset in the Environment pane.
...
Visualize the relation between happiness and GDP. Edit this visualization see see whether this differs by Region?
...
ggplot(happy, aes(x=GDPperCapita, y=Happiness, ___=___)) +
  geom_point()
...
ggplot(happy, aes(x = GDPperCapita, y = Happiness, color = Region, ___ = ___)) +
  geom_point()
...
#Let's fix it!
happy %>% 
  ___(!is.na(Happiness), !is.na(GDPperCapita)) %>% 
  group_by(Region) %>% 
