...
-   Collaborating within Posit Cloud and generating final RMarkdown report
...
Go to the course Posit Cloud Workspace and locate the R Studio Project for this Module, which should be named `Module 3`.
...
<!-- If working remotely through Zoom, make sure that the person taking the lead for an exercise is sharing their screen. -->
...
The first argument in the `if_else()` function we're using to write this if statement is the condition we're testing for. If `country` is equal to `"USA"`, we set `country_us` to `"USA"`. If not, we set the `country_us` to `"Other"`.
...
This means you'll need to define this data frame in your starter document, even though the next exercise doesn't explicitly ask you to do so.
...
Interpret your visualization, and say a few words about whether Buzzfeed's claim for this section is supported by the data.
...
Based on your visualization, do the data appear to support Buzzfeed's claim for this section? Explain your reasoning in 1-2 sentences.
