...
:::lo 
Open up `scripts/01-scrape-page-one.R` and start working in it.
:::
...
And this is how the text would look like on a webpage: [Sarch on Google](https://www.google.com).

Here the text is `Sarch on Google` and the `href` attribute contains the url of the website you'd go to if you click on the hyperlinked text: `https://www.google.com`.
...
library(knitr)
library(kableExtra)
fact_checks_lie <- read_csv("../nicksandbox/hw-lab-keys/data/fact_checks_lie.csv") # <<< you just created this .csv in the R scripts
fact_checks_truth <- read_csv("../nicksandbox/hw-lab-keys/data/fact_checks_truth.csv") # <<< you just created this .csv in the R scripts
...
Let's do a hypothetical: Say I had text in some column `rhyme` that goes like, "The cow jumped over the moon and the fiddle ran away with the spoon." 
...
In this example, the separate() function is used to split the "rhyme" column into three columns based on the words "jumped" and "and". Why three? Because I get everything before the word "jumped" in it's own column (I'm calling "before,") everything after the word "and" in it's own column (I'm calling "after"), and everything between in it's own column (you guessed it, that I'm calling "between"). And to wrap it all up, I'm just keeping the "between" column in my final data frame.

`r qbegin(12)`

Now, following this logic above, take our data frame and pipe it into a `separate()` function that will allow you to extract just the date and the media source from the `description` column. 
...
__Note__ that this will result in some warnings when you run the code, and that's OK for what we are wanting to do. 
...
Print out a summary of the data frame using the `skim()` function. How many unique sources are there? Do we have any variables with missing data? 

`r qend()`

`r qbegin(15)`

For the statements labeled as lies, display a rank-ordered table of the most common featured sources (ignore any sources that occur 2 or fewer times). 

Who (or what) is the most commonly featured source for the statements labeled as lies? Why do you think this source shows up so often?

`r qend()`
 
`r qbegin(16)`
 
Of the statements labeled lies, how many statements contain the word "Biden"? How many statements contain the word "Trump"? Try to figure it out, and ask for help if you're stuck.
 
`r qend()`

:::green
**Hint:** For Q16, `str_detect()` can be helpful here. 
:::
 
`r qbegin(17)`
 
Inspect the "media" variable on your own in the data set. Do any of the entries look incomplete? For incomplete entries, you might want to go to its page on the Politifact website to find what should have been captured. 

Can you speculate on why our code failed to capture complete entries for some of the statements?    
`r qend()` 
