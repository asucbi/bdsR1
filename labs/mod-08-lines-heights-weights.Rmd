...
1. Predict the value of $y$ for $x=3$ based on a regression model with intercept ($\alpha$) = -1 and slope ($\beta$) = .75
...
`r qbegin(10)`
Finally, create a residual plot and a plot of the predicted values against observed children's values using `plot_height_weight_preds()` for the new `fit_poly` model. What do you see?
