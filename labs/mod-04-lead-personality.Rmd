...
< introduction to effects of lead exposure on crime, etc. >
...
You can read the first of these in using the following, we'll label it `survey`.
...
The second dataset we will use is some country-level statistics for all European Union countries. You can read in the second dataset with the following, labeling it `eudata`:
...
`r qbegin(1)`
Look at the names of the variables in the `eudata` data. What do you think the data in each of these variables represents?
`r qend()`
...
To investigate whether lead exposure is related to personality, we need to join these data frames into one. But, first we need to recode the `Country` variables so that they match. Fortunately there are only two that are trouble. 
...
`r qbegin(5)`
Now we are ready to join the data frames. Join them on the `Country` variable.
`r qend()`
...
The `ggplot2` package includes a function called `map_data()` that loads datasets for plotting maps of different parts of the world. The code below loads data with latitude and longitude points marking the boundaries of all the different nations of the world and then makes that `data.frame` object into a `tibble`.
