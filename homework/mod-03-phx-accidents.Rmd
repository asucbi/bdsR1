...
First, open the R Markdown document `mod-03-hw.Rmd` and knit it.
...
The data is in two files and both are hanging out in some online repository. The first file is called `phoenix_accidents_1.csv` and the second file is called `phoenix_accidents_2.csv`. 
...
1.  How many observations (rows) do the two datasets have? Instead of hard coding the number in your answer, use inline code.

1.  Join `phx_accidents_1` and `phx_accidents_2` datasets into a single dataset called `phx_accidents`. Be sure to join `by=` the common variable between the two datasets.  

The variable (column) descriptions are as follows. Read through them, try to get a sense of what you have available:
...
1.  Run `View(phx_accidents)` in your Console to view the data in the data viewer. What does each row in the dataset represent?
...
phx_accidents %>%
  ...
  # 1. record the lecture 4 to show how to pivot
  # 2. use a mutate of some sort but 
  
  # they should know how to do a mutate but the lecture so far has never introduced them to an "if_else" sort of statement with %in% ... either future lectures needs to be adjusted or the assignment here modified. just keep an eye on things if this is assumed knowledge for other hws/labs  
  ...
  # to know how to use theme_minimal they need access to the 4th lecture
  theme_minimal() +
  
  # i do not believe they were ever introduced to this scale_fill_viridis_d() option. check with Derek. but this needs to be removed or explained in a lecture.   
  # scale_fill_viridis_d() +
  
  # were they introduced on how to do things using "labs"
  labs(
