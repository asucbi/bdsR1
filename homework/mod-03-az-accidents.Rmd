...
```{marginfigure}
Notice this .csv file is stored in an online database. The `read_csv` function is able to retrieve this file just as long as you give it the correct URL. 
```
...
phx_accidents <- 
  read_csv("https://asucbi.github.io/data/phoenix_accidents.csv") %>% 
  select(day_of_week, severity, time)
```
