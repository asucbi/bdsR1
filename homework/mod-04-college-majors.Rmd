...
Go to the course RStudio Cloud Workspace and locate the folder for the homework assignment, which should be named `hw-04-college-majors`.
...
4. We can also calculate summary statistics for this distribution using the `summarize` function:
...
college_recent_grads %>%
  group_by(major_category) %>%
  summarise(typical_median = median(median)) %>% # one possible choice
...
🧶 ✅ ⬆️  *When you are finished, save and knit your document and confirm the outputs look as you expect them to.*
