...
Go to the course Posit Cloud Workspace and locate the folder for the homework assignment, which should be named `hw-04-college-majors`.
...
1.  Using what you've learned so far, arrange the data in descending order with respect to proportion of women in a major, and display only the major, the total number of people with major, and proportion of women. Show only the top 3 majors by using the `head()` function at the end of the pipeline.
...
5. Which major category has the highest typical (you'll need to decide what this means) median income? Use the partial code below, filling it in with the appropriate statistic and function. Also note that we are looking for the highest statistic, so make sure to arrange in the correct direction.
...
6. Which major category is the least popular in this sample? To answer this question we can use a function called `count`, which first groups the data and then counts the number of observations in each category (see below). Add to the pipeline appropriately to arrange the results so that the major with the fewest observations is on top.
